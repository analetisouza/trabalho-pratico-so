...
int* state; //verify if the philosopher is THINKING, HUNGRY or EATING
int* current_philosopher; //return the id of the current philosopher
...
void put_chops(int* pos_current_philosopher){
    ...
    printf("Philosopher %d put the %d chopstick on the table\n", *pos_current_philosopher, *pos_current_philosopher);
    printf("Philosopher %d got the %d chopstick\n\n", *pos_current_philosopher, ((*pos_current_philosopher) + 1) % total_philosophers);
...
{
    int i;
    // recebe como parâmetro de entrada na linha de comando: Número de Filósofos, Tempo de pensamento, Tempo comendo
